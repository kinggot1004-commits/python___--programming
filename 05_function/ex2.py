...
#  2. 함수 인자 전달 방식
# ===========================================================
# 1) call-by-value: 값 자체가 복사되어 전달, 원본과 별도의 메모리 공간을 할당(다름), 함수 내부에서 수정 시 원본 불변
# 2) call-by-reference: 변수의 주소가 전달, 원본과 같은 메모리 공간을 가리킴(같음), 함수 내부에서 수정 시 원본 변경
...
print(a,b) # 전역변수 a,b 출력

def append_item(num): #num 매개변수가 객체의 참조값 복사
...
def num_info(*args):
    a = (min(args),max(args),sum(args),sum(args)/len(args))
    return a
