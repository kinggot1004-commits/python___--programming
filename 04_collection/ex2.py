...
result2 = ori.copy()
result2[0].append(10) #주소값을 그대로 가져온다, 그래서 리스트 안 리스트는 같이 변한다
...
print([zip(subjects, scores)]) # [x] x 자체를 원소 하나로 취급해 넣음
print(list(zip(subjects, scores))) # list(x) x를 차례대로 순회해 리스트에 넣음
print(["python"])
print(list("python"))
# ===========================================================
#  List Comprehension
...
name = ["pororo","crong","poby","eddy"]
# 6 5 4 4
result = [ len(x) for x in name]
print(result)
# 길이가 5 이상인 이름만 뽑기
result = [ x for x in name if len(x) >= 5]
print(result)

# 중첩 for문도 가능
x = [0,1,2]
y = [0,1,2]
result = [ i * z for i in x for z in y] # for문 사이에도 if가 들어갈 수 있다
print(result)
